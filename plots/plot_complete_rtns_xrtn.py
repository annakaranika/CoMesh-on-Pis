...
def getComplete(dir_prefix):
    ...
    complete = [[0 for _ in rs] for _ in vs]
...
if __name__ == "__main__":
    ...
    for vi, comp in enumerate(complete):
        for ri, r in enumerate(rs):
            ax.bar(
                x + (width + 0.15) * vi, comp, width, color='white',
    ...
    print(f"Saving complete routines' plot at plots/complete_rtns_{dims}_xrtn{log_txt}.png")
    plt.savefig(f'plots/complete_rtns_{dims}_xrtn{log_txt}.png')