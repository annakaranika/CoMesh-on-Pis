...
if __name__ == "__main__":
    ...
    fn_chunk = f'{shape}{dims_str}_v{virtual_dim}_n3_{pos}'
    virtual_dim = [int(v) for v in virtual_dim.split(',')]

    ip_prefix = "192.168.4."
    ip_ends = [15, 17, 20] + list(range(100, 100 + math.prod(dims) - 3))

    node_fn = f"workloads/node_list/node_list_{fn_chunk}.txt"
    with open(node_fn, "w+") as node_f:
        for hub_ip_end in ip_ends[:3]:
            node_f.write(f'{ip_prefix}{hub_ip_end}_0000 100\n')
    ...
    hub_ips = [f"{ip_prefix}{ip_end}_0000" for ip_end in ip_ends[:3]]
    simple = [dev_id for dev_id in device_locations.keys() if dev_id not in hub_ips]
    filename = f"workloads/dev_topo/dev_topo_{fn_chunk}.txt"
    with open(filename, 'w+') as f:
        for hub_ip in hub_ips:
            other_hubs = [other for other in hub_ips if other != hub_ip]
            f.write(f"{hub_ip} {' '.join(other_hubs)} {' '.join(simple)}\n")
        for dev_id in simple:
            f.write(f"{dev_id} {' '.join(hub_ips)}\n")
